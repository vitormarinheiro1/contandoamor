        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-2">
              ...
              <span className="text-sm font-medium text-primary">Contando Amor</span>
            </div>
            ...
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-border/50 shadow-xl">
              {data.photos.map((photo, index) => (
                <img
                  ...
                  className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-1000 ${index === currentPhotoIndex ? "opacity-100" : "opacity-0"
                    }`}
                />
              ))}
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {data.photos.map((_, index) => (
                    <button
                      ...
                      className={`h-2 w-2 rounded-full transition-colors ${index === currentPhotoIndex
                          ? "bg-white"
                          : "bg-white/50"
                        }`}
